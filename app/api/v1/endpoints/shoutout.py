...
@router.get("/my", summary="Get user shoutouts page by page",
            response_model=shoutout_schema.ShoutoutsOut)
async def get_user_shoutouts(current_user: user_schema.UserOut = Depends(user_crud.get_current_user),
                             ...
                             db: Session = Depends(get_db)):
    list_user_shoutouts, shoutouts_count = shoutout_crud.read_user_shoutouts(db=db,
                                                                             user_id=current_user.id,
                                                                             page_limit=page_limit,
                                                                             page=page)
    shoutouts_out = shoutout_crud.get_shoutouts_out(db_shoutouts=list_user_shoutouts)
    return {"shoutouts": shoutouts_out, "shoutoutsCount": shoutouts_count}
